...
class Car:
    ...
    """ 2. This Function: this fuction is responsable for drawing the map for the AI to go around it is using
    things such as self draw radar to draw the vechials raidar onto the screen/display for the AI car to visulise and use 
    to get arounf the track. 
    ...
    """ 3. This Function: This function is drawing the green lines onto the screen. This function is also getting the infomation from position = radar 0
    and using it to get around the track. There is also self center which is the center of the object that the radars are coming off. 
    ...
    """ 4. This Function: This function is cheaking the postition of the car. In the IF staitment the code is cheking if the color of the pixle of the car matches 
    up with the border of the map. If the border of the map maches with the car then the Self.alive = false will destory the car.  
    ...
    """ 5. This Function: This function is used to alert the car of the walls on the map. This function calulates the distance from the radar on the car to the wall 
    and moves the car the coresponding and suitable way using the X and Y coordinates. This allows the car to try and make it around the track best as possible. 
    ...
    """ 6. This Function: This function is the brains of the car and how it gets around the track. This code calulates the speed of the car and sets it 
    depending on the distance of the radars. It also has a self checking system where it checks for collisions between the object (car) and the map. 
    The car tells when it needs to turn by checking how many pixels it is away from differnt edges on differnt sides. 
    ...
    """ 7. This Function: This function is for the radars it is calulation the position of the car depending on the data from the radars. The data from the radar is 
    being turned into a integer and being divided by 30. The value is the stored and used by the car.
    ...
    """ 8. This Function: This Function: This function is telling the car weather it is alive or dead. If self alive = true then the car will keep using data from the previous 
    functions if no the car will stop taking inputs. 
    ...
    """ 9. This Function: This functions is Calulating the distance from the car to the wall using the size of the car. The fuction is calulating a reward which is used to 
    reinforce or discorage certian behaviours. The reward is related to the distance from the car to the wall in relation to its size. 
    """
    ...
    """ 10. This Function: This function is the orentation of the car image. It takes a image and an angle as a input and
    it returnes a rotated virsion of the inout image 
